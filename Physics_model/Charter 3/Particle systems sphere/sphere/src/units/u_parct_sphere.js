...
import { directionVectorInSphere, gaussianRandom, directionVectorAngulOfsetNormal, getRandomArbitary } from "../math/mathrandom.js";
...
let R = 10;
export function FreeParctInSphere(){
    ball = [];
    n = 0;
}

export function initParctInSphere() {
  let MtlNo2 = material.add(material.set(
    ...Matlib.Obsidian,
    [Tex.create("neptune.png"), -1, -1, -1, -1, -1, -1, -1],
    0,
    "texture"
  ));
...
export function renderParctInSphere() {
  ...
  if (f > 1){
    for(let i = 0; i < 2; i++){
      ...
      let Vn = directionVectorAngulOfsetNormal(u, D2R(45));
